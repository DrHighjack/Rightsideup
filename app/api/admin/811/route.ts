    ...
    if (action === 'create') {
      ...
      if (adminEmail) {
        try {
          await sendEmail({
            to: adminEmail,
            subject: `New 811 Ticket Created: ${ticketNumber}`,
            html: `
              <h3>New 811 Ticket</h3>
              <p><strong>Ticket Number:</strong> ${ticketNumber}</p>
              <p><strong>From:</strong> ${sourceEmail}</p>
              <p><strong>Subject:</strong> ${emailSubject}</p>
              <p><strong>Address:</strong> ${parsedAddress || 'Not parsed'}</p>
              <p><a href="${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin/811/${ticket.id}">View Ticket</a></p>
            `,
          });
        } catch (emailError) {
