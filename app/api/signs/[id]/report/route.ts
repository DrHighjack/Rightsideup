...
export async function POST(
  ...
  try {
    ...
    const emailContent = `
      <h2>Sign Report: ${type}</h2>
      <p><strong>Sign Number:</strong> ${sign.signNumber || "N/A"}</p>
      <p><strong>Report Type:</strong> ${type}</p>
      <p><strong>Description:</strong> ${description}</p>
      <p><strong>Reported by:</strong> ${user.name || user.email}</p>
      <p><strong>Sign Status Updated To:</strong> ${newStatus}</p>
      <p>
        <a href="${signLink}" style="background-color: #4f46e5; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; display: inline-block;">
          View Sign Details
        </a>
      </p>
    `;
    ...
    try {
      await sendEmail({
        to: adminEmail,
        subject: `[ALERT] Sign Report: ${type} - ${sign.signNumber}`,
        html: emailContent,
      });
    } catch (emailErr) {
