...
export default function RealtorOnboardingBanner({ status, completing, onSkip }: RealtorOnboardingBannerProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-950/40 p-4 backdrop-blur-sm sm:p-6"
      role="presentation"
      onMouseDown={(event) => {
    ...
    >
      <div
        className="w-full max-w-3xl rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-sky-50 p-5 shadow-2xl sm:p-6"
        role="dialog"
        aria-modal="true"
        aria-labelledby="realtor-onboarding-title"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 id="realtor-onboarding-title" className="text-2xl font-semibold text-blue-900">Welcome to North Shore Sign Co 👋</h2>
          ...
          <button
            ...
            onClick={onSkip}
            disabled={completing}
            className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-blue-200 text-lg leading-none text-blue-700 hover:bg-white disabled:cursor-not-allowed disabled:opacity-60"
