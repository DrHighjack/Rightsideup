    ...
    for (const email of emails) {
      try {
        ...
        if (parsed.confidence === 'low' || !parsed.address) {
          ...
          await sendEmail({
            to: adminEmail,
            subject: `⚠️ 811 Ticket Needs Review: ${parsed.ticketNumber || '(unknown)'}`,
            html: `
              <h2>811 Ticket - Manual Review Required</h2>
              <p><strong>Ticket:</strong> ${parsed.ticketNumber || '(not extracted)'}</p>
              <p><strong>From:</strong> ${email.from}</p>
              <p><strong>Subject:</strong> ${email.subject}</p>
              <p><strong>Parsed Address:</strong> ${parsed.address || '(not found)'}</p>
              <p><strong>Reason:</strong> Low confidence extraction - please review manually</p>
              <p><a href="${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin/811/${ticket.id}">Review Ticket</a></p>
            `,
            text: `811 Ticket Needs Review\nTicket: ${parsed.ticketNumber}\nFrom: ${email.from}\nAddress: ${parsed.address || '(not found)'}\n\nPlease review at /admin/811/${ticket.id}`,
          });
        ...
        await sendEmail({
          to: adminEmail,
          subject: `📍 811 Ticket Created: ${parsed.ticketNumber} (${matchedOrderIds.length} orders held)`,
          html: `
            <h2>811 Ticket Alert</h2>
            <p><strong>Ticket Number:</strong> ${parsed.ticketNumber || '(not extracted)'}</p>
            <p><strong>Address:</strong> ${parsed.address}</p>
            <p><strong>Work Start Date:</strong> ${parsed.workStartDate || '(not extracted)'}</p>
            <p><strong>From:</strong> ${email.from}</p>
            <p><strong>Status:</strong> ${ticketStatus}</p>
            <hr />
            <h3>Matched Orders: ${matchedOrderIds.length}</h3>
            ${
              matchedOrderIds.length > 0
                ? '<p>The following orders have been placed ON_HOLD:</p><ul>' +
                  orders
                    .filter((o) => matchedOrderIds.includes(o.id))
                    .map((o) => `<li>${o.orderNumber} - ${o.address}</li>`)
                    .join('') +
                  '</ul>'
                : '<p>No orders matched. This ticket has been flagged for review.</p>'
            }
            <p><a href="${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin/811/${ticket.id}">View Ticket Details</a></p>
          `,
          text: `811 Ticket: ${parsed.ticketNumber}\nAddress: ${parsed.address}\nMatched Orders: ${matchedOrderIds.length}`,
        });
